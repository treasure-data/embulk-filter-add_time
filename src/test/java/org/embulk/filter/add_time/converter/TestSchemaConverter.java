...
public class TestSchemaConverter
{
    ...
    public void validate()
    {
        ...
        {
            ConfigSource conf = this.config.deepCopy().set("from_value", ImmutableMap.of("mode", "upload_time"));
        ...
        // the type of the column specified as from_column must be 'long', 'timestamp' or 'string'
        {
            ConfigSource conf = this.config.deepCopy()
                    ...
                    .set("from_column", ImmutableMap.of("name", "c0"));
            try { // boolean
                new SchemaConverter(log, pluginTask(conf), schema("c0", Types.BOOLEAN));
                fail();
            }
            catch (Throwable t) {
                assertTrue(t instanceof ConfigException);
            ...
            try { // double
                new SchemaConverter(log, pluginTask(conf), schema("c0", Types.DOUBLE));
                fail();
            }
            catch (Throwable t) {
                assertTrue(t instanceof ConfigException);
